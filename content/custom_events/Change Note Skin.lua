...
local songArrowSkin = 'noteSkins/NOTE_assets'
...
function onCreate()
	...
	if getPropertyFromClass('PlayState', 'SONG.arrowSkin') == nil or getPropertyFromClass('PlayState', 'SONG.arrowSkin') == 'SONG.arrowSkin' then
		songArrowSkin = 'noteSkins/NOTE_assets'
	else
		...
		if getPropertyFromGroup('eventNotes', i, 'event') == 'Change Note Skin' then
			...
			for iii = 0, getProperty('eventNotes.length') - 1 do
				if not (iii == i) and getPropertyFromGroup('eventNotes', iii, 'event') == 'Change Note Skin' then
					...
					if (character == 'bf' and (string.find(getPropertyFromGroup('eventNotes', iii, 'value1'), 'bf') 
					or string.find(getPropertyFromGroup('eventNotes', iii, 'value1'), 'boyfriend')))
					or (character == 'dad' and string.find(getPropertyFromGroup('eventNotes', iii, 'value1'), 'dad')) 
					or character == 'both' then
