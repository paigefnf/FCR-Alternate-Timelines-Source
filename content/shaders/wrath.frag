...
uniform float uShadeColor1;
uniform float uShadeColor2;
uniform float uShadeColor3;
uniform vec3 uOverlayColor;
uniform float uOverlayColor1;
uniform float uOverlayColor2;
uniform float uOverlayColor3;
...
uniform float applyRect1;
uniform float applyRect2;
uniform float applyRect3;
uniform float applyRect4;
...
float texture2DAlphaCheck(vec2 uv) {
	if(uv.x >= applyRect1 && uv.y >= applyRect2 && uv.x <= applyRect3 && uv.y <= applyRect4) {
...
vec4 flixel_texture2DShaded(sampler2D bitmap, vec2 uv) {
	...
	if(uv.x >= applyRect1 && uv.y >= applyRect2 && uv.x <= applyRect3 && uv.y <= applyRect4) {
		...
		vec3 shading = (fshading) * vec3(uShadeColor1,uShadeColor2,uShadeColor3);

		vec3 finalColor = blendMultiply(
			color.rgb +
				uPower * blendLinearLight(shading.rgb, color.rgb, (1.0 - color.a)),
			vec3(uOverlayColor1,uOverlayColor2,uOverlayColor3), uOverlayOpacity
		);
